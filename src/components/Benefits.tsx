...
interface Slide {
  type: 'hero' | 'text' | 'list' | 'split' | 'grid' | 'benefits';
  title: string;
  subtitle?: string;
  tag?: string;
  content?: string;
...
const slides: Slide[] = [
  ...
  },
  {
    ...
    content:
      'Somos Terabound, un equipo con más de 25 años de experiencia desarrollando tecnología para industrias complejas: petróleo, banca, seguros, logística, gastronomía y muchas más.',
  ...
  },
  {
    ...
    content:
      'Después de décadas resolviendo problemas reales, entendimos que el verdadero valor no estaba en cada solución individual, sino en la relación entre ellas.',
    items: [
  ...
  },
  {
    ...
    content:
      'Diseñar ecosistemas digitales que respondan a las necesidades reales de las empresas. No solo a lo que pueden expresar, sino a lo que verdaderamente necesitan para operar mejor.',
  ...
  },
  {
    ...
    content:
      'Ser la empresa de referencia en el diseño de ecosistemas digitales para industrias complejas. Reconocidos por nuestra comprensión profunda del negocio.',
  ...
  },
  {
    ...
    content:
      'Nos tomamos el tiempo de entender lo que realmente necesita cada empresa, más allá de lo que puede expresar en un primer encuentro.',
  ...
  },
  {
    ...
    content:
      'Diseñamos soluciones que reducen la dependencia del papel, eliminan procesos innecesarios y ayudan a que las empresas operen de forma más eficiente.',
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-12 items-center">
      ...
      <Card className="bg-white/10 border-white/20 p-10 backdrop-blur-xl opacity-100">
        <div className="mb-6 text-emerald-400">{Icon && <Icon size={48} />}</div>
...
function SplitSlide({ slide }: { slide: Slide }) {
  return (
    <div className="text-center max-w-4xl mx-auto">
      ...
      <div className="text-3xl font-bold text-emerald-400 italic">"{slide.footer}"</div>
    </div>
  );
}
...
function BenefitsSlide({ slide }: { slide: Slide }) {
  return (
    <div>
      <div className="text-center mb-16">
        <SlideTag>{slide.tag}</SlideTag>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {slide.items?.map((benefit: any, index: number) => {
...
export function Benefits() {
  ...
  return (
    <section
      aria-label="Manifiesto y valores de Terabound"
      className="py-32 bg-[var(--primary)]/90 text-white overflow-hidden relative min-h-[700px] flex items-center"
    >
        ...
        <div className="relative max-w-5xl mx-auto">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              aria-label={`${currentSlide + 1} de ${slides.length}`}
